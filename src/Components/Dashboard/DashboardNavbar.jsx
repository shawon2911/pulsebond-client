...
const DashboardNavbar = () => {
 ...
 const mounted = useSyncExternalStore(
  ...
  const pageTitle = currentRoute
    ? currentRoute.replaceAll("-", " ").replace(/\b\w/g, (l) => l.toUpperCase())
    : "Dashboard";
  ...
  const dashboardItems = {
    donor: [
      ...
      { icon: RiProfileFill, label: "Profile", link: "/dashboard/profile" },
      {
        icon: MdRequestPage,
  ...
  // 🌟 মাউন্ট হওয়ার আগে প্রথম অক্ষর 'U' ডিফল্ট থাকবে, মাউন্ট হলে ইউজারের প্রথম অক্ষর নিবে
  const firstLetter = mounted && user?.name ? user.name.charAt(0) : "U";
    ...
    <div className="flex justify-between items-center px-5">
      <Drawer>
        ...
        <div className="w-full flex items-center justify-between md:hidden border-b border-white/10">
          <Drawer.Trigger>
            <div className="flex items-center gap-2 text-ink bg-white/10 hover:bg-white/20">
            ...
            <Dropdown.Trigger className="rounded-full">
              <Avatar size="sm" aria-label="Menu">
                <Avatar.Image
                  ...
                  alt={user?.name || "User Dropdown"}
                  src={user?.image}
                />
                {/* 🎯 FIXED: Dynamic Fallback standard string handle text check */}
